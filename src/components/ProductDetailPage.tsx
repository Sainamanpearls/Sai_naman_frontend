...
import { SIZES, COLORS } from '../data/productVariants';
...
export default function ProductDetailPage({
  ...
  const handleAddToCart = () => {
    ...
    const productForCart: Product = {
      ...
      color: product.hasColor ? selectedColor : null,
    };
    ...
    console.log(productForCart.color);
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {product.images.map((image, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-20 sm:w-24 aspect-square border-2 overflow-hidden ${selectedImage === index ? 'border-white' : 'border-zinc-800 hover:border-zinc-600'
                        } transition-all`}
                    >
            ...
            <div className="flex flex-col space-y-6 lg:w-1/2">
              ...
              {product.hasSize && (
                <div>
                  <h3 className="text-zinc-400 text-sm tracking-wider mb-2">SIZE</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {SIZES.map((size) => (
                      <button
                        key={size}
                        onClick={() => setSelectedSize(size)}
                        className={`px-4 py-2 border text-sm tracking-wider w-full text-center transition ${selectedSize === size
              ...
              {product.hasColor && (
                <div>
                  <h3 className="text-zinc-400 text-sm tracking-wider mb-2 mt-4">COLOR</h3>
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                    {COLORS.map((color) => (
                      <button
                        key={color}
                        onClick={() => setSelectedColor(color)}
                        className={`px-4 py-2 border text-sm tracking-wider w-full text-center transition ${selectedColor === color
                            ? 'border-white text-white'
                            : 'border-zinc-700 text-zinc-400 hover:border-zinc-500'
                          }`}
