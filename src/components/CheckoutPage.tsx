...
export default function CheckoutPage({ items, onOrderComplete }: CheckoutPageProps) {
  const pageTitle = 'Checkout - Sai Naman Pearls';
  const pageDescription =
    'Secure checkout for your favorite items. Review your order, enter shipping details, and complete your payment safely.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const options = {
        ...
        prefill: {
          name: `${formData.customerName} ${formData.customerLastName}`,
                  ...
                  {[
                    ...
                    <div key={field}>
                      ...
                      <input
                        ...
                        className={`w-full bg-black border border-zinc-800 px-4 py-3 text-white focus:border-white focus:outline-none transition-colors ${field === 'customerEmail' ? 'opacity-60 cursor-not-allowed' : ''
                          }`}
                      />
                    </div>
                  ))}
            ...
            <div className="space-y-6">
                  ...
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {/* Show discounted price if it exists */}
                        {item.discountedPrice ? (
                          ...
                          <p className="text-white mt-1">₹ {(item.price * item.quantity).toLocaleString()}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-6">
                <div className="flex items-start space-x-3 text-sm text-zinc-400">
                  ...
                  <p>
                    All pieces are carefully packaged in our signature gift box with authenticity
                    certificate.
                  </p>
                </div>
              </div>
