...
export default function UserOrdersPage({ onBack }: UserOrdersPageProps) {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const current = data.filter(
        (o: Order) => o.status === 'pending' || o.status === 'processing' || o.status === 'shipped'
      );
  ...
  const copyOrderNumber = (order: Order) => {
    const id = order.display_id || order._id.slice(-8).toUpperCase();
    navigator.clipboard.writeText(id);
    toast.success(`Order #${id} copied!`, { theme: 'dark' });
  };
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-xl text-white font-light tracking-wider flex items-center space-x-2">
                <span>Order #{order.display_id || order._id.slice(-8).toUpperCase()}</span>
                <Copy className="w-4 h-4 cursor-pointer text-zinc-400 hover:text-white" onClick={() => copyOrderNumber(order)} />
              </h3>
              <span
            ...
            <div className="mb-4">
              {order.display_id ? (
                <a
                  href={`https://shiprocket.co/tracking/order/${order.display_id}?company_id=4370755`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-zinc-900 text-white py-2 px-4 border border-zinc-700 hover:bg-zinc-800 transition-colors text-sm tracking-wider"
                >
                ...
                <button
                  disabled
                  className="inline-block bg-zinc-800 text-zinc-500 py-2 px-4 border border-zinc-700 cursor-not-allowed text-sm tracking-wider"
                >
                  <Truck className="w-4 h-4 inline mr-1 opacity-50" />
            ...
            {/* Total Amount & Expand */}
            <div className="flex items-center justify-between pt-4 border-t border-zinc-800 cursor-pointer" onClick={() => fetchOrderItems(order._id)}>
    ...
    <div className="min-h-screen bg-black px-4 py-12 pt-32">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-5xl font-light tracking-widest text-white mb-2">MY ORDERS</h1>
            <p className="text-zinc-500 tracking-wide">Track and manage your orders</p>
          </div>
          <button onClick={onBack} className="text-zinc-400 hover:text-white transition-colors tracking-wider">← BACK TO HOME</button>
        </div>
        ...
        <div className="flex space-x-4 mb-8 border-b border-zinc-800 overflow-x-auto">
          <button
            ...
            className={`pb-4 px-6 tracking-wider transition-all whitespace-nowrap ${activeTab === 'all' ? 'text-white border-b-2 border-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            <Package className="w-5 h-5 inline mr-2" /> ALL ({allOrders.length})
          </button>
          <button
            onClick={() => setActiveTab('current')}
            className={`pb-4 px-6 tracking-wider transition-all whitespace-nowrap ${activeTab === 'current' ? 'text-white border-b-2 border-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            <Clock className="w-5 h-5 inline mr-2" /> CURRENT ({currentOrders.length})
          </button>
          <button
            onClick={() => setActiveTab('completed')}
            className={`pb-4 px-6 tracking-wider transition-all whitespace-nowrap ${activeTab === 'completed' ? 'text-white border-b-2 border-white' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            <CheckCircle className="w-5 h-5 inline mr-2" /> COMPLETED ({completedOrders.length})
          </button>
        </div>
