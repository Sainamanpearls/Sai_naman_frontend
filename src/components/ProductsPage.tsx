          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {categories.map((category) => (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full border text-sm tracking-wide transition-all ${selectedCategory === category.slug || selectedCategory === category.id
              ...
              <div className="space-y-2">
                {categories.map((category) => (
                  <div key={category.id} className="relative flex items-center">
                    <button
                      onClick={() => handleCategoryClick(category.slug || category.id!)}
                      className={`flex-1 text-left px-4 py-3 rounded-md transition-all duration-300 font-light text-sm sm:text-base ${selectedCategory === category.slug || selectedCategory === category.id
                    ...
                    {isAdmin && category.id !== 'all' && (
                      <button
                        ...
                        className={`ml-2 p-2 rounded-md transition-all ${deleteConfirm?.id === category.id
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto">
                <select
                  ...
                  className="bg-gray-900/60 backdrop-blur-md border border-gray-800/50 text-gray-200 px-3 py-2 rounded-md focus:border-gray-600 focus:ring-2 focus:ring-gray-700/30 tracking-wide outline-none w-full sm:w-auto"
                >
                    ...
                    >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className={`absolute top-4 left-4 p-2 rounded-md transition-all ${deleteConfirm?.id === product.id
                              ? 'bg-red-600 text-white'
                              : 'bg-gray-900/80 text-gray-400 hover:bg-red-600 hover:text-white'
                            }`}
                        >
